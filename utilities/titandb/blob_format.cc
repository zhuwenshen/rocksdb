...
void BlobFileMeta::FileStateTransite(const FileEvent& event) {
  switch (event) {
    case FileEvent::kFlushCompleted:
      // blob file maybe generated by flush or gc, because gc will rewrite valid
      // key to memtable, and the memtable may be flushed before gc finish and
      // change its file state. If it's generated by gc, we will leave gc to
      // change its file state. If it's generated by flush, we need to change it
      // to normal state.
      assert(state == FileState::kPendingLSM || state == FileState::kPendingGC);
      if (state == FileState::kPendingLSM) state = FileState::kNormal;
      break;
    case FileEvent::kGCCompleted:
      assert(state == FileState::kPendingGC || state == FileState::kBeingGC);
      state = FileState::kNormal;
      break;
    case FileEvent::kCompactionCompleted:
      assert(state == FileState::kPendingLSM);
      state = FileState::kNormal;
      break;
    case FileEvent::kGCBegin:
      assert(state == FileState::kNormal);
      state = FileState::kBeingGC;
      break;
    ...
    default:
      fprintf(stderr,
              ...
              static_cast<int>(event), static_cast<std::size_t>(file_number),
